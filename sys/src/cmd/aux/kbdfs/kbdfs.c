...
/*
 * Line editor for cons.
 *
 * Plan2001: stock kbdfs only cooks with backspace, ^U (whole line), ^W and
 * ^D.  This adds cursor editing and history:
 *
 ...
 * ESC[nD) and ESC[K, which the kernel console (sys/src/9/pc/vga.c) and any
 * serial terminal understand, and which stay correct when a line wraps onto
 * several rows.  Every write starts with ESC[m (a no-op) so the kernel
 * console does not log editing echo as kernel messages (devcons.c).
 * The kernel's help bar lists these keys.
 */
enum {
...
struct Line {
	Rune	r[Nline];
	int	n;		/* runes in the line */
	int	cur;		/* cursor, 0..n */
	int	ccur;		/* where the console's cursor is, relative to the line start */
};

static Rune cutbuf[Nline];
static int ncut;
static Rune pend[Nline];	/* the line being typed while browsing history */
static int npend;
static Rune *hist[Nhist];
static int hlen[Nhist];
static int nhist;

static char obuf[Nout+8] = "\033[m";
static int nobuf = 3;
...
omove(int d)
{
	...
	if(d < 0)
		oput(b, snprint(b, sizeof b, "\033[%dD", -d));
	else if(d > 0)
		oput(b, snprint(b, sizeof b, "\033[%dC", d));
}

/* one rune, one cell: tab and control characters are shown as a single glyph */
static void
orune(Rune r)
{
	char b[UTFmax];
	...
	if(r == '\t')
		...
		r = '?';
	oput(b, runetochar(b, &r));
}

/* make the screen show l->r[from..n) (from < 0: text unchanged) and put the cursor at l->cur */
static void
sync(Line *l, int from)
{
	int i;
...
/* remove l->r[a..b); if cut, keep it for ^Y */
static void
delrange(Line *l, int a, int b, int cut)
{
	if(cut){
		ncut = b - a;
		memmove(cutbuf, l->r+a, ncut*sizeof(Rune));
	}
	memmove(l->r+a, l->r+b, (l->n-b)*sizeof(Rune));
	l->n -= b - a;
}

static int
insert(Line *l, Rune *p, int n)
{
	if(l->n + n > Nline-1)
		n = Nline-1 - l->n;
	if(n <= 0)
		return 0;
	memmove(l->r+l->cur+n, l->r+l->cur, (l->n-l->cur)*sizeof(Rune));
	memmove(l->r+l->cur, p, n*sizeof(Rune));
	l->n += n;
	return n;
}
...
addhist(Line *l)
{
	...
	if(l->n == 0)
		return;
	if(nhist > 0 && hlen[nhist-1] == l->n && memcmp(hist[nhist-1], l->r, l->n*sizeof(Rune)) == 0)
		return;
	if((p = malloc(l->n*sizeof(Rune))) == nil)
		return;
	memmove(p, l->r, l->n*sizeof(Rune));
	if(nhist == Nhist){
		free(hist[0]);
		memmove(hist, hist+1, (Nhist-1)*sizeof(hist[0]));
		memmove(hlen, hlen+1, (Nhist-1)*sizeof(hlen[0]));
		nhist--;
	}
	hist[nhist] = p;
	hlen[nhist] = l->n;
	...
	for(;;){
		...
		do {
			...
			case Kcutend:	/* ^K: cut cursor .. line end */
				...
				break;
			case Ketb:	/* ^W: cut the word before the cursor */
				i = l.cur;
				while(i > 0 && (l.r[i-1] == ' ' || l.r[i-1] == '\t'))
					i--;
				while(i > 0 && l.r[i-1] != ' ' && l.r[i-1] != '\t')
					i--;
				if(i < l.cur){
					delrange(&l, i, l.cur, 1);
					l.cur = i;
					from = i;
				}
				break;
			...
			case Kup:
				if(hi > 0){
					if(hi == nhist){
						npend = l.n;
						memmove(pend, l.r, l.n*sizeof(Rune));
					}
					hi--;
					l.n = hlen[hi];
					memmove(l.r, hist[hi], l.n*sizeof(Rune));
					l.cur = l.n;
					from = 0;
				}
				break;
			case Kdown:
				if(hi < nhist){
					...
					if(hi == nhist){
						l.n = npend;
						memmove(l.r, pend, l.n*sizeof(Rune));
					}else{
						l.n = hlen[hi];
						memmove(l.r, hist[hi], l.n*sizeof(Rune));
					}
...
threadmain(int argc, char** argv)
{
	...
	/*
	 * Plan2001: we are the start of interactive use.  ESC[2J makes the
	 * kernel console (sys/src/9/pc/vga.c) clear itself and switch from the
	 * multi-column boot log to a full-width scrolling console with a help
	 * bar; serial terminals just clear.
	 */
	write(1, "\033[2J", 4);
