...
/*
 * Kernel entry, from the UEFI loader (sys/src/boot/efi).  This is the first
 * text symbol, so it is the entry address in the a.out header.
 *
 * We are in 64-bit long mode already: the firmware runs with paging on, all
 * memory identity mapped, and this text executes at its physical address
 * (link addresses minus KZERO).  Load our GDT and segments, build the boot
 * page tables, switch to them and continue at _start64v.
 *
 * The loader has checked that CR4.LA57 is clear (the paging depth cannot be
 * changed in long mode) and that the firmware's own page tables lie outside
...
TEXT _efi64<>(SB), 1, $-4
	...
	/* reload CS with our 64-bit code segment */
	PUSHQ	$SELECTOR(KESEG, SELGDT, 0)
	MOVL	$_efi64b<>-KZERO(SB), AX
	PUSHQ	AX
	RETFQ
...
memloop:
	...
	/*
	 * Switch to the new tables.  Long mode and PAE are on already (long
	 * mode cannot run without them); page size extensions are off, page
	 * global on, and CR0 gets write protect and loses the cache-disable,
	 * not-write-through, task-switched and monitor bits.  This code runs
	 * at its physical address, which the double map covers.
	 */
	MOVQ	SI, CR3				/* load the mmu */
	DELAY
	...
	/* out of the identity map, to the kernel's virtual address */
...
/*
 * Long mode, at the kernel's virtual address.
