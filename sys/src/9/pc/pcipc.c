...
enum
{	/* configuration mechanism #1 - universal on every PCI chipset since
	 * the early 1990s, long before UEFI; the only mechanism this kernel
	 * knows.  Configuration mechanism #2 (byte-wide, deprecated by the
	 * PCI spec itself, pre-dates mechanism #1) and BIOS32/PCI BIOS
	 * (pcibios.c, no BIOS32 service directory exists under UEFI) are not
	 * implemented: there is no PCI-capable, UEFI-only machine that needs
	 * either. */
...
pcicfginit(void)
{
	...
	/*
	 * Configuration mechanism #1: a DWORD at 0xCF8 (PciADDR) and another
	 * at 0xCFC (PciDATA). Universal on PCI hardware; the only mechanism
	 * this kernel implements (see the enum above for what is not).
