...
{
	VGAscr *scr;
	int x, y, z, stride, tilt;
	uvlong pa;
	ulong chan, sz;
	char *s, *p, *err;

	/*
	 * *bootscreen=WIDTHxHEIGHTxSTRIDExDEPTH CHAN PA [SZ]
	 *
	 * The old WIDTHxHEIGHTxDEPTH form remains accepted with STRIDE=WIDTH.
	 */
	...
	*p = ' ';
	...
	if((err = setscreensize0(scr, stride, y, z, chan, tilt)) != nil){
		print("setscreensize0: %s\n", err);
		return;
	}
	scr->width = x;
	setactualsize(scr, Rect(0, 0, x, y));
...
{
	char conf[128], chan[30];
	int stride;

	conf[0] = '\0';
	if(scr != nil && scr->paddr != 0 && scr->gscreen != nil){
		stride = scr->pitch * 8 / scr->gscreen->depth;
		snprint(conf, sizeof(conf), "%dx%dx%dx%d %s 0x%.8llux %d\n",
			scr->width, scr->height, stride, scr->gscreen->depth,
			chantostr(chan, scr->gscreen->chan), scr->paddr, scr->apsize);
